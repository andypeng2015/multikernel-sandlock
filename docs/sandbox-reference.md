...
| `max_open_files` | `open_files`  | `int \| None`           | `None`  | Maximum number of open file descriptors. Enforced via `RLIMIT_NOFILE` (kernel, survives `exec`), set in the child right before it execs. Both the soft and the hard limit are lowered, so the sandbox cannot raise the cap back; descendants inherit it. Clamped to the hard limit sandlock itself inherited — it is an upper bound, never a grant. The limit must also cover process startup (stdio, the dynamic loader's per-library descriptors, and under `chroot` the injected exec fd); too low a value fails the exec with `EMFILE` and exits 127. Measured floors for a trivial command: 4 for a plain dynamically linked exec, 17 under `chroot`; programs linking more libraries need more. |
